...
# macOS sed requires -i '', GNU/Linux sed requires -i with no arg
...
# Detect if we're running on the deployment server (has kubectl + runner)
is_deploy_server() {
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo "  4. Ensure env files are on the deployment server"
...
# Uses macOS Keychain on Darwin, file-based on Linux
KEYCHAIN_SERVICE="trpc-template-godaddy"
SECRETS_DIR="$HOME/.config/trpc-template/secrets"
...
store_keychain() {
  ...
  if [[ "$(uname)" == "Darwin" ]]; then
    ...
    echo "$value" > "$SECRETS_DIR/$account"
    chmod 600 "$SECRETS_DIR/$account"
  fi
}
...
read_keychain() {
  ...
  if [[ "$(uname)" == "Darwin" ]]; then
    ...
    cat "$SECRETS_DIR/$account" 2>/dev/null
  fi
}
...
if [ -z "$GODADDY_API_KEY" ] || [ -z "$GODADDY_API_SECRET" ]; then
  ...
  echo "  GoDaddy credentials saved."
  echo ""
  DEFAULTS_CHANGED=true
fi
...
find . -type f \( -name "*.json" -o -name "*.ts" -o -name "*.tsx" -o -name "*.vue" -o -name "*.yml" -o -name "*.yaml" -o -name "*.md" -o -name "*.js" -o -name "Dockerfile" \) \
  ...
  -print0 | xargs -0 sed "${SEDI_PREFIX[@]}" "s|@template/|${SCOPE}/|g"

find . -type f \( -name "*.yml" -o -name "*.yaml" \) \
  ! -path "*/node_modules/*" ! -path "*/.git/*" \
  -print0 | xargs -0 sed "${SEDI_PREFIX[@]}" "s|<REPO_NAME>|${PROJECT_NAME}|g"

sedi "s|\"trpc-template\"|\"${PROJECT_NAME}\"|g" package.json

sedi "s|template_db|${DB_NAME}|g" docker-compose.yml
sedi "s|template_user|${DB_USER}|g" docker-compose.yml
sedi "s|template_pass|${DB_PASSWORD}|g" docker-compose.yml
...
sedi "s|TEMPLATE_APP_NAME|${APP_DISPLAY_NAME}|g" packages/mobile/app.json
sedi "s|TEMPLATE_SLUG|${PROJECT_NAME}-mobile|g" packages/mobile/app.json
sedi "s|TEMPLATE_SCHEME|${APP_SCHEME}|g" packages/mobile/app.json
sedi "s|TEMPLATE_SCHEME|${APP_SCHEME}|g" packages/mobile/src/lib/auth.ts
sedi "s|art.tylermakes.TEMPLATE_BUNDLE|art.tylermakes.${BUNDLE_ID_SUFFIX}|g" packages/mobile/app.json
sedi "s|TEMPLATE_DOMAIN|${PROJECT_DOMAIN}|g" packages/mobile/app.json
sedi "s|TEMPLATE_DOMAIN|${PROJECT_DOMAIN}|g" packages/mobile/app/_layout.tsx
sedi "s|TEMPLATE_AUTH0_DOMAIN|${AUTH0_DOMAIN}|g" packages/mobile/app.json
sedi "s|TEMPLATE_AUTH0_CLIENT_ID|${AUTH0_CLIENT_ID}|g" packages/mobile/app.json
sedi "s|TEMPLATE_AUTH0_AUDIENCE|${AUTH0_AUDIENCE}|g" packages/mobile/app.json
...
# ─── Step 4: Ensure env files are on the deployment server ───────────
step "4/11" "Verifying env files are in place..."

# Env files were already written to $LOCAL_ENV_DIR in step 3.
# If we're on the deployment server, they're already where they need to be.
if is_deploy_server; then
  ok "Running on deployment server — env files already at ${LOCAL_ENV_DIR}/"
else
  warn "Not running on the deployment server (no kubectl found)."
  info "Copy these files to ~/envs/ on the deployment server before deploying:"
  info "  ${LOCAL_ENV_DIR}/${PROJECT_NAME}.env"
  info "  ${LOCAL_ENV_DIR}/${PROJECT_NAME}-postgres.env"
fi
...
" 2>/dev/null)
  ...
  if [ -n "$EAS_PROJECT_ID" ]; then
    ...
    sedi "s|TEMPLATE_EAS_PROJECT_ID|${EAS_PROJECT_ID}|g" packages/mobile/app.json
  ...
  if [ -f "$INGRESS_FILE" ]; then
    if grep -qF "$PROJECT_DOMAIN" "$INGRESS_FILE"; then
      ...
      if ! grep -q "proxy-read-timeout" "$INGRESS_FILE"; then
        python3 -c "
import re
with open('$INGRESS_FILE') as f: t = f.read()
t = t.replace('  annotations:\n', '''  annotations:
    nginx.ingress.kubernetes.io/proxy-read-timeout: \"3600\"
    nginx.ingress.kubernetes.io/proxy-send-timeout: \"3600\"
    nginx.ingress.kubernetes.io/proxy-connect-timeout: \"3600\"
''', 1)
with open('$INGRESS_FILE', 'w') as f: f.write(t)
"
      ...
      python3 -c "
with open('$INGRESS_FILE') as f: t = f.read()
t = t.replace('    secretName:', '        - ${PROJECT_DOMAIN}\n    secretName:', 1)
with open('$INGRESS_FILE', 'w') as f: f.write(t)
"
