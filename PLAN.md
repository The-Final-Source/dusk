...
### Files to create:
- `packages/hooks/package.json` — deps: `@trpc/client`, `@trpc/react-query`, `@tanstack/react-query`, `react`; devDeps: `@dusk/api`, `@dusk/shared`, `@types/react`, `typescript`
...
### Verify:
- `pnpm install && pnpm --filter @dusk/hooks build` compiles cleanly
...
- `package.json` — remove vue/vue-router/@auth0/auth0-vue/vue-tsc/@vitejs/plugin-vue/@tanstack/vue-query; add react/react-dom/react-router-dom/@auth0/auth0-react/@dusk/hooks/@vitejs/plugin-react/tailwindcss/@tailwindcss/vite
...
- `src/views/Users.tsx` — uses `useUsers()` from `@dusk/hooks` for CRUD

### Files to modify:
- `Dockerfile` — add `COPY packages/hooks/...` lines and `pnpm --filter @dusk/hooks build` step
...
- `app/(tabs)/users.tsx` — uses `useUsers()` from `@dusk/hooks` (shared with web)
