...
- Set verify: "structural" on a triple whose truth is the PRESENCE/SHAPE of a declaration or config file (a manifest field, a tsconfig option, a config wiring) — it is checked mechanically (anchor resolves + decoration coverage), zero LLM, and converges immediately. Leave verify off (defaults "semantic") for any triple about the BEHAVIOR of code an evaluator must read and judge. Litmus: if you cannot point at a concrete declaration/region whose mere existence makes the triple true, it is semantic. Do NOT put a behavioral triple (about runtime/source behavior) in a config intent and mark it structural — author it where its subject actually exists. See skill dusk/author/verify-channel.
