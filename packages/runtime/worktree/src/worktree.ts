 ...
 * an isolated `git worktree add -b dusk/<bead-id> <path> origin/main`; the rest of
...
/** Create one isolated worktree for a bead off the base ref (default `origin/main`). */
export function addWorktree(repoDir: string, beadId: string, options: AddWorktreeOptions = {}): RuntimeResult<WorktreeHandle> {
  ...
  const baseRef = options.baseRef ?? WORKTREE_BASE_REF;
