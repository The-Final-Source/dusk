...
The Decomposer's unresolved-intent escalation path SHALL continue to write the `ImplementCheckpoint` shape unchanged per Phase 3 (the JSON shape is frozen — Phase-3 D4). **The `suggested_dialog_seed` *content* SHALL be enriched** from the Phase-3 naive `unresolved_refs.join(", ")` to a Stage-1 framing prompt produced by the pure function `enrichDialogSeed(unresolvedRefs, snapshot, request?)` (per design D4, signature corrected by arch-board 2026-06-11 D6). Parent, sibling, and `relates_to`-linked context come FROM THE SESSION SNAPSHOT; the original request is supplied by the caller as the third argument — the snapshot is a `DerivedIndex` and does not carry per-call request text. The Decomposer escalation path SHALL pass the originating request (`decompose.ts` passes `input.request`); the request-less fallback exists only for callers that genuinely have no request context and SHALL produce the explicit degraded framing ("An intent … is referenced but doesn't exist"), never fabricated framing. The shape stays frozen — only `suggested_dialog_seed`'s string content evolves. (RFC §10.1.1; design D4; Phase-3 `bead-decomposition` modification.)
