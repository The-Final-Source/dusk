...
- The `payload` parameter SHALL be REJECTED (tombstoned). It is removed from the supported contract — `ResolveLivelockRequestSchema` in `@dusk/core-schema` carries no `payload` and the runtime `resolveLivelock(report, verb)` signature drops it — but it remains DECLARED in the MCP `inputSchema` (with a tombstone `.describe()`) solely so Phase-3-form callers receive the typed `DuskError { kind: "config_invalid" }` rejection with a `recovery_hint` pointing at `dialog_init`, instead of having the argument silently stripped at the transport layer and proceeding as a bare `modify_triple`. Any non-`undefined` `payload` with ANY verb SHALL be rejected before verb dispatch. The tombstone declaration may be dropped at the next major MCP surface revision. (Arch-board 2026-06-11 D3.)
