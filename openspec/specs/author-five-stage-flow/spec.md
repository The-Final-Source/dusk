...
Stage 4.5 SHALL invoke `validateMatrixPredicateNegation`, `validateAntecedentGrammar`, `validateRelatesToKinds`, and `validateAtomicIntent` from `@dusk/core-parser`. These four functions are named entry points ADDED IN PHASE 4 (`packages/core/parser/src/validate.ts`) as thin adapters over the Phase-1 single-source implementations — `findTripleNegations`/`findIllegalNegation`, the closed `ANTECEDENT_PREDICATES` / `RELATES_TO_KINDS` vocabularies, `isResolvableReference`, and `loadIntent` — required because the Phase-1 typed surfaces accept the strict `Intent` and cannot take relaxed drafts. NO RULE CONTENT is reimplemented: every rule has exactly one source, shared with the parser gate (`loadIntent`) and the PreToolUse gate, so drift between Author and gate is impossible by construction. A violation SHALL bounce the transition back to Stage 4 with the violation's `code` and a skill-name hint (`polarity-decision` / `implies-antecedent-grammar` / `typed-relates-to`); the Author drafts a corrected triple and re-enters 4.5. (RFC §3.1.1, §3.2.1; design D3; arch-board 2026-06-11 D1.)
