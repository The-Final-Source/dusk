...
1. Replaces `@dusk/` placeholders with `@my-app/` across all source files, Dockerfiles, and manifests
2. Generates a strong database password
3. Creates `.env` files (local dev + deployment)
4. Ensures deployment env files are at `~/envs/` on the deployment server
