...
The mobile app shares the same `@dusk/hooks` package as the web app:

```typescript
// packages/mobile/app/_layout.tsx
import { TRPCProvider } from "@dusk/hooks";
...
```bash
pnpm --filter @dusk/mobile add -D jest @testing-library/react-native
```
...
- **Types out of date?** The mobile app imports from `@dusk/hooks` which imports types from `@dusk/api`. Run `pnpm build` to rebuild the type chain.
