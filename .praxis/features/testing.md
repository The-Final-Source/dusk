...
> **Note:** `@template` is a placeholder package scope. When you run `scripts/init.sh <project-name>`, it replaces every `@dusk/` reference (including in these docs) with `@<project-name>/`.
...
pnpm --filter @dusk/api test       # alternative filter syntax
```
