...
```typescript
import { useUsers } from "@dusk/hooks";
...
1. Install test dependencies: `pnpm --filter @dusk/web add -D vitest @testing-library/react @testing-library/jest-dom jsdom`
...
- **Types out of date?** Run `pnpm build --filter=@dusk/shared --filter=@dusk/api --filter=@dusk/hooks`. The web package reads types from the compiled output of other packages.
