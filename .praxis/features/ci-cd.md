...
2. Creates K8s secret from env file (`~/envs/<repo-name>.env`)
...
1. Loads `VITE_*` env vars from env file (`~/envs/<repo-name>.env`)
2. Builds Docker image with Vite build args, tagged with git SHA
3. Applies K8s deployment manifest
4. Waits for rollout (120s timeout)

### deploy-infra.yml
Triggers on changes to `.k8s/**`. Manages the PostgreSQL secret (created from `~/envs/<repo-name>-postgres.env`) and deploys the PostgreSQL K8s deployment. Waits for rollout (120s timeout).
...
- **Deploy succeeded but app is broken?** Check `kubectl logs deployment/<repo-name>-api` on the deploy server. The rollout status check only verifies pods are running, not that the app is healthy.
...
- **All workflows queue sequentially?** The self-hosted runner processes one job at a time per runner. Workflows across all repos queue. This is by design.
...
- **K8s secret issues?** The env file must exist at `~/envs/<repo-name>.env` on the runner. If it's missing, `init.sh` wasn't run for this project.
