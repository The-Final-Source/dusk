...
### [Web App](./web-app.md)
React 19 SPA built with Vite + TailwindCSS v4. React Router v7 for navigation. `AuthGuard` component protects routes. tRPC hooks from `@dusk/hooks` for all data fetching. Views: Home, Users, Profile, Admin.
...
Expo Router (React Native) with tab navigation. Auth0 via `expo-auth-session` with SecureStore tokens. Shares the same `@dusk/hooks` tRPC hooks as web. Scaffolds for push notifications, calendar access, and file downloads.
