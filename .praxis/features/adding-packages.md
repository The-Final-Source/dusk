...
1. **`workspace:*` in package.json** — pnpm symlinks the package into `node_modules`, so `import { UserSchema } from "@dusk/shared"` resolves at runtime.
...
import { UserSchema, type User } from "@dusk/shared";

// NO — reaching into internal files
import { UserSchema } from "@dusk/shared/src/schemas/user.js";
```
...
- **`workspace:*`** — tells pnpm to use the local version of `@dusk/shared`
- **`exports`** — required for other packages to import from `@dusk/worker`
...
- **`references`** — list every internal package this one imports from. If you use `@dusk/shared`, add `{ "path": "../shared" }`.
...
> **Exception:** `api` has `composite: true` because it exports types via its `./client` subpath (`@dusk/api/client`). If your server package similarly exposes types consumed by other packages, it needs `composite: true` too.
...
Then run it from the root: `turbo worker:process --filter=@dusk/worker`.
