...
import { CreatePostSchema } from "@dusk/shared";
...
- **Type errors in the client?** The type chain is: schema → router → AppRouter type → hooks. Check that your router is added to `appRouter` in `routers/index.ts` and that `@dusk/shared` is rebuilt (`pnpm build --filter=@dusk/shared`).
