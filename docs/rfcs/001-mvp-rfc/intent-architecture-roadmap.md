...
**Greenfield foundations are authored intents, not pipeline machinery (arch-board, RFC App. D.24–D.27).** The build's first surfaced friction was an endpoint intent authored before its foundation existed, forcing one bead to birth the whole app. The resolution keeps the thesis pure: project/module + tech-stack setup, app bootstrap, and the persistence layer are the **first sequence of authored intents** — never a synthesized foundation bead or canonical Block. The Author dialog gains a general Stage-2 `prerequisite` tension class (the request depends on an absent intent; the greenfield foundation is its canonical instance) that steers the author to write those prerequisite intents first — surfaced through the existing tension flow, so no bootstrap state enters the orchestration; the pipeline gains no greenfield special-case. Alongside it, the native greenfield/external-repo load drove tightly-scoped runtime robustness (external-repo gate-path + base-ref resolution; "a wall-clock overrun on the Engineer is a continue, not a discard" — salvage the partial draft and re-enter, no turn cap) — D11-style support fixes, reshaping no capability.

The build also surfaced a genuine capability gap (RFC App. D.28): inline decoration only reaches comment-bearing code, so comment-less files (e.g. `package.json`) could not be linked to intents — total decoration was not actually total. Its resolution is **universal decoration coverage** — a per-file `<filename.ext>.intent` sidecar (structural JSON-Pointer anchors, derived/enforced line coverage), a dusk-level `decoration.ignore` glob set as the only exemption, and a keystone fix that finally wires the `.intent` sidecar into the index/Verifier/doctor. This is its **own v1.x OpenSpec change and a prerequisite** for a fully decoration-covered greenfield POC — built before the POC restarts from scratch.
...
**v1.x sequencing note:** the v1.x changes specified so far, in order — **(1) universal decoration coverage** (RFC App. D.28; the `<file>.intent` sidecar + `decoration.ignore` SSoT + the keystone index fix), the prerequisite that lands before the POC; then **(2) the Greenfield POC** itself (Sprint 11 above; Phase 6 in the implementation plan), which depends on (1) for full coverage of its comment-less files. The POC build is the thing *surfacing* further v1.x correctives — a series of arch-board-resolved gaps in shipped v1 behavior (App. **D.29/D.30/D.31** the structural verification channel; **D.32 `test-pyramid-routing`** — route test intents by authored suffix, not the decoration marker; **D.33 `model-call-failure-handling`** — a deterministic `error_max_turns` model-call failure is classified non-transport and surfaced as a returned failure, never cold-retried to a fatal crash; **D.34 `boundary-outcome-handling`** — the *governing principle* (RFC §1.2.1): the deterministic core acts only on signals it authored (intent/decoration/flow/clock) or that are universal (do bytes parse into Dusk's *own* schema), the agentic bridge interprets the tech surface (a tool's output meaning), and every boundary crossing resolves to exactly one of `content | no_verdict | transport` — never inferring a verdict from silence. The live driver: an empty/degraded Verifier under load was mistreated as a content reject and looped ~21× re-drafting correct code (the earlier "Stage-2 vitest-flake" framing was wrong and is superseded). As the 4th instance of the D.26→D.33 lesson, the principle is *extracted* — not patched a fourth time — and a mechanical floor keeps it honest without re-coupling to a toolchain: the project's test command emits Dusk's own result schema via a project-side adapter, the core reads only that schema, and `no_verdict` routes to a finite, resumable infrastructure-recovery axis) — each landing as its own change **before the POC build proceeds past the layer that surfaced it**. None requires any item on *this* list — in particular none pulls legacy bootstrap forward; greenfield is v9's native mode and needs no retro-decoration machinery. Everything below remains demand-triggered, with the POC's exploratory friction data as the primary prioritization input. (These are closed gaps, not deferrals — each ships as its own change.)
