...
3. **Dusk owns the pipeline.** Lifecycle hooks (PostToolUse for source-map maintenance) are gone. The PreToolUse gate enforces decoration. The full 9-step pipeline (request → bead DAG → worktrees → pair programming → shuffle sampling → tests → commit → merge → return) runs under Dusk's orchestration.
...
| **Tool scoping advisory in v1** | Proposal §9.4 | Role-frontmatter `tools:` is honored as configuration, NOT as a hard sandbox. PreToolUse gate provides the actual safety boundary. Hardening deferred to v1.x. |
