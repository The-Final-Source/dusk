...
### 1.2.1 The deterministic core is tech-agnostic; agentic AI bridges the tech surface (NEW v9 — App. D.34)

Dusk is a **higher-level interface for building software**: a deterministic orchestration core driving a **universal flow** onto **any stack**. The core must never couple to a specific technology's nuances — it does not, and must not, know about vitest, tsc, pytest, or any toolchain's vocabulary. This is a governing tenet, not an aspiration:

> The **deterministic core** may compute control flow **only** over signals it **itself authored** (intent syntax, intent files, the decoration grammar, the orchestration flow, its own clock/timeout) or that are **universal across every stack** (whether returned bytes parse into Dusk's *own* schema). Anything that requires **judgment about what a specific tool's output means** — that is the *project's* tech surface area, not Dusk's own — **must be interpreted by agentic AI**, never hard-coded into the core. The core orchestrates across any stack; **agentic AI is the bridge**, re-emitting opaque, stack-specific output as a Dusk-authored signal.

A corollary governs every boundary crossing (a real-model leg; an opaque-tool-output boundary): it resolves to **exactly one** of **`content`** (a Dusk-schema-valid *and* content-complete verdict — the only outcome that may drive re-draft / commit), **`no_verdict`** (empty / unparseable / incomplete / deterministic-limit / tool-infrastructure — a distinct recovery axis that never becomes a content fail, a silent pass, a crash, or a consumed content-iteration), or **`transport`** (call-plumbing — retried once, then escalates to `no_verdict`). The core never infers a verdict from the *absence* of a negative signal ("inferring verdict from silence"); it requires **positive** success evidence. Two honesty duals hold **model-independently**: no false re-draft loop on correct code, **and** no masking of a genuine failure. The signals that are genuinely stack-invariant are few — only Dusk's own timeout firing and "bytes parse into Dusk's own schema," and even those are *liveness* facts, never *content*. Full treatment, the signal-reliability verdict, and the requirements (R1–R11): **App. D.34**.
...
**Stage-2 distinguishes a content verdict from an infrastructure `no_verdict` (NEW v9 — App. D.34).** The Test Runner core never parses a tool's vocabulary (no reading vitest's `success`/`numFailedTests`, no inferring failure from a non-zero exit or a `status!=="passed"`). It captures only `{command, exitCode, stdout, stderr, timedOut}` and resolves the run to **`content`** (a Dusk-schema-valid, content-complete verdict — `pass`/`fail`), **`no_verdict`** (the suite did not yield Dusk's result schema — reporter crash / OOM / timeout / truncated output), or **`transport`**. A content `decision:"fail"` drives re-draft and **blocks commit**; a `no_verdict` routes to the finite infrastructure-recovery axis and **never** becomes a content fail, a silent green, a crash, or a consumed content-iteration. The mechanical floor that keeps this honest *without* coupling to a toolchain: the project's test command emits results in **Dusk's own result schema** via a thin **project-side adapter/reporter** (configured in the foundation intent — the project's tech surface), and the agentic bridge interprets only output that did not yield that schema. See §1.2.1 and App. D.34.
...
**D.33 (resolved — arch board, Phase 6 greenfield POC; investigate→debate→converge).** **A deterministic, content/limit-shaped model-call failure must be classified as non-transport and SURFACED as a returned failure the recovery ladder handles — never cold-retried as transient plumbing and never an uncaught fatal crash.** This is the **D.26 lesson generalized from the Engineer path to the model-call path** (the model client backing the Verifier, the Stage-1 test-body pre-pass, and the non-Engineer reasoning sub-agents). The POC build crashed: a no-tools JSON model call (`claude --print … --max-turns 3 --disallowed-tools …`) burned its turn budget attempting tools (`stop_reason:"tool_use"`, `num_turns:4`) and exited `error_max_turns` (non-zero); `runClaude` rejected with `"claude CLI exited 1: …"`; `isTransportError` matched `/claude CLI (timed out|exited)/` and classified it **transport**; `withTransportRetry` cold-retried the *identical deterministic* call (it reproduced exactly), and the second "transport death" threw `TransportLegFailure` — **uncaught** on every model-call caller path (the `taskRunner` ELSE branch, `verifyIntent`, and the test pre-pass all `await modelClient.complete` with no try/catch; the spawn seam `spawnSubAgent` awaits `taskRunner`/`verifierFactory` with no try/catch; the short cycle handles only *returned* failures, never a throw) → `dusk implement` exit 1, no bead merged. **Two facets, resolved from first principles (NOT by transferring D.26's salvage mechanism — a model call has no partial artifact to salvage, unlike the Engineer's worktree; `stop_reason:"tool_use"` yields no verdict):**
- **Facet B — handling (the correctness fix; two load-bearing parts).** (i) **Honest classification at the source.** `runClaude`, on a non-zero exit whose stdout parses to a CLI result envelope carrying an error subtype (`error_max_turns` and siblings), tags the thrown error structurally (a `duskModelExit` field) so `isTransportError` returns `false` for it — a deterministic limit failure is no longer cold-retried (retrying an identical deterministic call is waste, and is what manufactured the fatal second death). Classification lives at the throw site where the *structured* `subtype` is in hand, NOT as a new alternation in the stringly-typed `TRANSPORT_MESSAGE_RE` regex (a structural tag also avoids the `runtime-verifier → test-harness` dependency cycle a typed-error import would create). (ii) **Surface, don't crash, at the spawn seam.** `spawnSubAgent` wraps the `taskRunner`/`verifierFactory` awaits in a **narrow** try/catch that converts *only a classified model-call/CLI failure* (transport- or limit-shaped) into a returned `verifier_model_call_failed` (verifier/pre-pass) / `task_tool_call_failed` (reasoning) — the existing recoverable signals the short cycle and recovery ladder already route — so the failure flows to a legible `implement: <kind> — <message>` outcome instead of a stack-trace crash. **The catch is narrow by mandate:** programming errors (`TypeError`, assertion failures, `internal_error`) MUST propagate and fail loud — mirroring `isTransportError`'s own rule ("programming errors are never bookkept as model noise"); a bare catch that relabels every throw `verifier_model_call_failed` would trade a loud crash for a silent false-recovery and is rejected. **Narrowing the classifier ALONE is a non-fix** — once `error_max_turns` is non-transport, `withTransportRetry` rethrows the raw error immediately and it is *still* uncaught → still exit 1; both (i) and (ii) are required.
- **Facet A — source (defense-in-depth, a rate-improver only, zero correctness weight).** The no-tools call attempts tools because `--disallowed-tools` is a **denylist** that registers the tools in the request (the model sees them, attempts, is denied, re-attempts → turn burn) and is provably **incomplete** vs the CLI's real built-in set (a non-listed tool is a genuine, non-denied `tool_use`). Replace it with `--tools ""` — a zero-tool **allowlist** the CLI supports — so `tool_use` is structurally impossible (modulo MCP/future-tool/prompt-injection edge cases, which is exactly why this carries no correctness weight; the correctness rests entirely on Facet B). `--max-turns 3` is **kept** as a pure blast-radius backstop (its D.26 carve-out as "bounding stray tools" stands — D.33 refines, not reverses, D.26: the cap is fine *as a bound*; what D.26 omitted is that *hitting* the bound must be non-fatal and surfaced), and the code comment whose premise ("the model recovers and answers in text") was falsified is rewritten to say so. No model-facing change is ever a correctness guarantee. **Orthogonal to D.29–D.32** (this lives in the model-call error-handling path, not the verification logic). Its own v1.x OpenSpec change (`model-call-failure-handling`); lands before the Phase-6 POC resumes.

**D.34 (resolved — arch board, Phase 6 greenfield POC; investigate→debate→converge→re-audit). The corrective `boundary-outcome-handling` landed and archived 2026-06-17; this records the converged position/mandate.** **Dusk's deterministic core may drive control flow only on signals it authored or that are universal; everything that requires judgment about what a specific tool's output *means* is the agentic bridge's job — and every boundary crossing resolves to exactly one of `content | no_verdict | transport`, never inferring a verdict from silence.** This is the governing principle elevated in §1.2.1, surfaced by the POC and the **4th instance** of the D.26→D.33 lesson (a non-deterministic / infrastructure failure mistreated as a genuine/retryable content failure → futile loop or crash) — by the Rule of Three, the principle is extracted, not patched a fourth time.

**Corrected diagnosis (the original "Stage-2 vitest worker-pool flake" framing was WRONG and is superseded).** A (test) bead looped ~21 short-cycle entries re-drafting *correct* code. Verified against preserved traces (bead `bd_…005`, the 75-trace looper): the **Test Runner was never reached**; 30 of 34 short-cycle **Verifier** calls returned 0-token (empty/degraded) responses at 25–74 s under extreme concurrent load. The **confirmed live trigger** (materialization step 0) was the degraded response **silently coerced into a definite, falsely-converging verdict** — the completeness collapse `answer?.… ?? false` / `?? "vague"` ([procedure.ts:124,133] on the semantic path; the identical `genuinely_verifies ?? false` at [testPrepass.ts:87] on the live Stage-1 test-intent pre-pass the looping test bead actually routed through) — which slipped through the short cycle's `!verdict || isDuskError` guard as valid *content* and converged on an empty failing set. The twin `parseModelResponse → null → recoverable: true` **downgraded to `recoverable: false`** at the short-cycle/Stage-1/long-cycle boundaries ([loop.ts:172]) is a **real but latent** defect of the same family (D.33's catch never fires — the call *returns empty* rather than throwing). Fowler named the root smell: **"inferring verdict from silence"** — reading a control-flow decision off the *absence* of a negative signal (`anyPass && !anyFail`, `?? false`, `status!=="passed" ⇒ fail`). The robustness sweep found **nine** such boundaries (the silent-green on a never-consumed Stage-2 `decision`; the empty-verdict downgrade on the short cycle, the Stage-1 pre-pass, and the long cycle; unguarded `JSON.parse`/`execFileSync` crashes; hard-coded vitest vocabulary; the degraded-completeness coercions on both the semantic and the test-pre-pass paths; the non-run-as-fail coercion) — closed by **eight code fixes plus one already-mitigated boundary** (an unguarded success-path parse, already caught + classified transport at the spawn seam) confirmed by a guard test.

**Resolution — the three-way outcome + a mechanical floor that stays tech-agnostic:**
- **The core captures only `{command, exitCode, stdout, stderr, timedOut}`** and routes only on Dusk-authored verdicts; it never branches on a toolchain's exit code, status vocabulary, or "output produced." Exit code lies (137 OOM ≡ 1 assertion ≡ 0-zero-tests ≡ 127 missing-binary, plus wrapper passthrough); "output produced" lies (a 0-token response *is* transport-layer output). Only **two** signals are universal, and only as liveness: Dusk's own timeout firing, and whether bytes parse into Dusk's *own* schema.
- **A mechanical floor preserves the honesty duals without re-coupling to a tool** (the crux): just as the project's *code* is decorated to map to Dusk's *intent* schema, the project's *test command* emits results in **Dusk's own result schema** via a thin **project-side adapter/reporter** (a vitest reporter, a pytest plugin — the project's tech surface, configured in the foundation intent). The core reads only that schema (`failed>0 ⇒ fail`; `passed>0 ∧ failed==0 ⇒ pass`) — fully regression-testable, zero tech-coupling. The **agent interprets only when that schema is absent** (reporter crashed / OOM / timeout / garbage) → `no_verdict`, or `fail` where it reads a genuine assertion failure in raw output. The agent can **never** downgrade a Dusk-schema assertion failure to a flake nor manufacture a green — the asymmetry that guarantees the "no masked failure" dual.
- **`no_verdict` routes to a distinct, finite recovery axis** (one counter, terminal *legible* pause, precedence `livelock > no_verdict > budget`), is **excluded from livelock reject-observations** (infra noise must not trip the 3-reject detector), is **resumable** (the existing freeze/resume seam), and **never** counts as "not-a-reject" in the long-cycle N=2 confirmation. A genuine content `fail` still drives re-draft and still blocks commit (the never-consumed Stage-2 `decision` is now consumed). **git** `execFileSync` stays out of scope — git exit codes are a *universal* orchestration signal, a deliberate, stated asymmetry.

The classifier + the `content|no_verdict|transport` union live in the `core/schema` leaf (one source of truth, extending `modelCallError.ts`; no new package); the new kind is `infrastructure_no_verdict`, the new bead status `paused_infrastructure`. The mechanical guards (parse-into-Dusk-schema, the completeness check, the Dusk-result-schema floor, the routing tables, the finite counters, consuming `decision`) are **zero-model and hold both duals regardless of model behavior**; flake-vs-failure judgment on schema-absent output is agentic. The project-side adapter is a **Phase-VI / project-side task**, not part of the dusk change. *(Full mandate, R1–R11, signal-reliability table, the nine-gap closure, and the materialization plan: the archived change `openspec/changes/archive/2026-06-17-boundary-outcome-handling/` — `design.md` (decisions + Review round), `specs/boundary-outcome-handling/spec.md`, `tasks.md`.)*
