...
- **Unmet prerequisite / foundation gap** — the framing presupposes foundational intents/decisions the tree does not yet contain (NEW v9 — App. D.25)

The user picks the resolution; the Author encodes it.

**Foundation-gap detection (NEW v9 — the greenfield-robustness seam, App. D.24/D.25).** Tension detection runs in both directions: against intents that *exist* (the four classes above) and against intents that *should exist but don't*. When a requested behavior intent presupposes a foundation the tree lacks — the canonical greenfield case, where the tree is empty or has no project/module + tech-stack setup, app bootstrap, or persistence-layer intents — the Author surfaces this as a **foundation gap** rather than drafting an endpoint that would silently force the implementation to birth the whole app. The Author then steers the user to author the prerequisite foundation intents *first* (in dependency order: project/stack → bootstrap → persistence → behavior), each through this same dialog. This is the proactive, authoring-time complement to D.10 (the Decomposer's *reactive* mid-`dusk_implement` missing-intent pause): the gap is caught before code is ever requested. It is deliberately a **dialog-agent** responsibility, not an orchestration phase — the pipeline gains no greenfield special-case (App. D.24); the foundation is simply the project's first intents. A deterministic signal (the touched component's decoration/intent index is empty for the presupposed support surface) backs the Author's judgment so the prompt does not have to guess.
...
**D.25 (resolved — arch board, Phase 6).** **The Author dialog detects an unmet foundation / prerequisite gap and steers the user to author it first.** D.10 already handles a *reactive* missing intent (the Decomposer hits an intent that does not exist mid-`dusk_implement` → pause → Author → resume). D.25 makes the Author *proactive at authoring time*: during framing/discovery (Stage 1–2), when the requested behavior intent presupposes foundational intents/decisions absent from the tree (the canonical greenfield case: an empty or foundation-less tree), the Author surfaces the gap and guides authoring the prerequisite foundation intents first — an extension of the same Stage-2 tension machinery from "conflicts with an existing intent" to "depends on a missing one." This is the dialog-agent robustness that lets greenfield "just work" without coupling the pipeline to bootstrap (D.24). It lands as Author role-prompt/skill guidance plus a deterministic foundation-gap signal, not new orchestration.
