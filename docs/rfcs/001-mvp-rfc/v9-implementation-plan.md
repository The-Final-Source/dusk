...
- **The greenfield foundation IS authored intents, not an orchestration phase (App. D.24/D.25).** Project/module + tech-stack setup, app bootstrap, and the persistence layer are the **first sequence of intents** the author writes through `dusk_author` — never a synthesized foundation bead, bootstrap spawn, or canonical foundation Block (all rejected as orchestration coupling / framework tar-pit). The build authors **foundation intents first**, in dependency order (project/stack → bootstrap → persistence → behavior), then behavior intents. Robustness lives in the **Author dialog**: Stage-2 tension detection gains a *foundation-gap* class (App. D.25; RFC Ch. 5) that catches, at authoring time, a behavior intent presupposing foundation the tree lacks — the proactive complement to D.10's reactive mid-`dusk_implement` pause. This is friction-feedback work (role-prompt/skill edits), exactly the kind this phase's cohesive landing already calls for.
