...
**The POC build then surfaces further v1.x correctives, each landing before the build proceeds.** Running the greenfield POC is exercising shipped v1 behavior on its native terrain for the first time, and is surfacing arch-board-resolved gaps that ship as their own standalone v1.x changes (not phases, no coverage-matrix rows): App. **D.29/D.30/D.31** (the structural verification channel — channel is a property of the claim, declared by the author) and App. **D.32 / `test-pyramid-routing`** (test intents are routed by their authored path suffix, not the decoration marker; the marker locates the test body; a missing marker fails loud + is gate-rejected). Each lands before the POC build proceeds past the layer that surfaced it — `test-pyramid-routing` (D.32) **landed and archived 2026-06-16**, so the POC resumes from the test-pyramid layer. This is the §6.5 "greenfield load surfaces a dusk defect → tightly-scoped fix + living-spec delta" loop operating as designed.
