...
A sixth phase — **Phase 6, the Greenfield POC** — is specified after Phase 5. It is **post-v1**, not part of the v1 gate, and it adds no rows to the coverage matrix. It is in this document because it follows the same phase discipline (one OpenSpec change, behavioral tests, a cohesive-landing gate) and because its shape was settled while the v1 plan was live: v1's dogfood validates decorate-at-authorship on existing code; Phase 6 validates the thesis in its pure form — an API application where every line was born through Dusk. **Phase 6 is not the first v1.x change** — it has a v1.x prerequisite, **universal decoration coverage** (RFC App. D.28; the "Post-v1 prerequisite" section just before Phase 6 below), which lands first and is what lets the POC reach the 100%-coverage bar.
...
- **`packages/delivery/pre-tool-use`** — PreToolUse hook handler. Accepts BOTH wire shapes on stdin: Claude Code's real payload `{hook_event_name, tool_name, tool_input}` and the internal `{tool, args}` shape (programmatic/test callers), normalized by `normalizeHookInput`. Block contract (verified empirically): a block EXITS 2 with the plain-text reason on **stderr and empty stdout** (the only signal Claude Code honors under every permission mode, incl. `acceptEdits`); approve exits 0 with empty stdout; process failure / malformed input → fail-safe block `hook_internal_error` (exit 2). A `--json` mode emits the structured `HookOutput` on stdout for programmatic callers. Runs **all 10 mechanical checks** including check 10. Structured rejection payloads per App. A.8. The same `runGate` core is reused by the headless engineer's post-hoc in-process gate (`gateWorktreeEdits`) — the engineer does NOT pass through the interactive hook. (Sprint 2; RFC §4.6, §4.6.1, App. A.8, A.10.)
...
- **P3-T24 · Gate-fail loopback re-drafts without spending a Verifier call.**
  - **WHAT** — A draft blocked by the engineer's mechanical gate (the post-hoc in-process `gateWorktreeEdits` over the worktree diff — sharing `runGate` with the interactive PreToolUse hook, but NOT the hook itself, which the headless engineer does not pass through) routes back to the Engineer to fix, and the Verifier is only spawned once the gate passes.
...
## Post-v1 prerequisite — Universal Decoration Coverage (v1.x change, lands before Phase 6)

**This is the first v1.x OpenSpec change** (`universal-decoration-coverage`), scaffolded after the Phase-5 archive and **before** the Phase-6 POC restarts. It is **not a phase** in the six-phase structure and adds no rows to the coverage matrix; it is a standalone capability change that closes a genuine gap v1 shipped with, and a hard prerequisite for the POC's 100%-coverage bar. Full design: RFC **App. D.28** + **Ch. 4.5.4**.

**The POC build then surfaces further v1.x correctives, each landing before the build proceeds.** Running the greenfield POC is exercising shipped v1 behavior on its native terrain for the first time, and is surfacing arch-board-resolved gaps that ship as their own standalone v1.x changes (not phases, no coverage-matrix rows): App. **D.29/D.30/D.31** (the structural verification channel — channel is a property of the claim, declared by the author), App. **D.32 / `test-pyramid-routing`** (test intents are routed by their authored path suffix, not the decoration marker; the marker locates the test body; a missing marker fails loud + is gate-rejected), App. **D.33 / `model-call-failure-handling`** (a deterministic `error_max_turns` / content-limit model-call failure is classified non-transport — not cold-retried — and surfaced as a returned `verifier_model_call_failed` / `task_tool_call_failed` the recovery ladder handles, never an uncaught fatal crash; `--tools ""` removes the tool surface as defense-in-depth), and App. **D.34 / `boundary-outcome-handling`** (the *governing principle* — RFC §1.2.1: the deterministic core acts only on signals it authored or that are universal; the agentic bridge interprets the tech surface; every boundary resolves to exactly one of `content | no_verdict | transport`, never inferring a verdict from silence. The 4th instance of the D.26→D.33 lesson, so the principle is extracted rather than patched again. Live driver: an empty/degraded Verifier under load mistreated as a content reject → ~21× re-draft of correct code, *not* a vitest worker-pool flake — that earlier framing was wrong and is superseded. A mechanical floor keeps both honesty duals — no false re-draft on correct code, no masked genuine failure — without re-coupling to a toolchain: the project's test command emits Dusk's own result schema via a project-side adapter/reporter; the core reads only that schema; schema-absent output is interpreted agentically and routes to a finite, resumable infrastructure-recovery axis with precedence `livelock > no_verdict > budget`). Each lands before the POC build proceeds past the layer that surfaced it — `test-pyramid-routing` (D.32) **landed and archived 2026-06-16**; `model-call-failure-handling` (D.33) **landed and archived 2026-06-16**; `boundary-outcome-handling` (D.34) **landed and archived 2026-06-17**. This is the §6.5 "greenfield load surfaces a dusk defect → tightly-scoped fix + living-spec delta" loop operating as designed.

**The gap.** v9's thesis is *total* decoration — every line links to the intents it serves. But decoration was implemented only as inline `// @intent` comments, which **comment-less formats cannot carry** (strict JSON — `package.json` — holds no comment). So total decoration was not actually total: files existed that could not be linked to intents, and nothing enforced their coverage.

**The resolution** (decoration board, 3 rounds, unanimous):
- **One model, multiple parsers** — inline, the directory `.intent`, and a NEW per-file `<filename.ext>.intent` sidecar all normalize to the same `DecorationRecord[]` → one derived index. The second modality is a third *parser*, not a parallel system.
- **Keystone fix first** — the existing directory `.intent` sidecar was parsed *only by the gate*; the three `.ts`-only index walkers never read it, so `.intent` records never reached `buildDerivedIndex` (invisible to Verifier/reverse-index/doctor). Collapse the three walkers into one shared `.intent`-aware scanner; land it first, with the keystone test committed **red-first** — it must fail against current code before the scanner lands (that red is the evidence the bug is real), then go green.
- **Structural anchor stored, line view derived** — a sidecar claim stores a JSON Pointer (RFC 6901; `""` = whole file), never a line number; spans are re-resolved every run, so coverage can't go stale and a dangling pointer is a hard `unresolved_anchor` finding. (This is why a sidecar is acceptable here even though D.11 forbids sidecars for comment-*bearing* code — that verdict stands; see D.11 ↔ D.28.)
- **`decoration.ignore` glob SSoT** — one ignore set in `dusk.config.yml` (built-in defaults + project additions), consumed identically by the gate, the coverage scanner, and `dusk doctor` (replacing the hardcoded `SKIP_DIRS`); the only whole-file exemption, distinct from the per-region `@intent-ignore` marker. Everything not glob-ignored must be fully covered or it hard-blocks at the gate.
- **Mechanical verification** — comment-less/config records are `verify: "structural"` and excluded from the semantic Verifier (a manifest carries no architectural triple); never blended into semantic adherence.
- **Scope:** JSON-only sidecars (whole-file `@intent-file` fallback otherwise); no general region map, no source-map/VLQ, no content-hash, one literal `schema_version: 1`.

**Delivery + acceptance.** Its own validated OpenSpec change (`openspec/changes/universal-decoration-coverage/` — proposal + design + 6 capability specs + tasks; `--strict` passes). All zero-model mechanical code + colocated unit tests, then one composing integration test, then archive. Done = the keystone test goes green (a `.intent`/sidecar record reaches `buildDerivedIndex`), a sidecar's pointer resolves to the correct derived span and survives a target reformat, an uncovered non-trivial line in a non-ignored file hard-blocks, structural records never enter the semantic prompt, and `pnpm build`/`test`/`typecheck` are green. It has no live-infra/model "e2e" — the integration test is the end-to-end.

**Why it gates Phase 6.** The POC produces its own `package.json`/config through Dusk; without the sidecar those files cannot be decoration-covered, so the POC could not hit "full coverage ALWAYS." This change lands first; then the POC restarts from scratch.

---

## Phase 6 — Greenfield POC (POST-V1 phase — the second v1.x change)

**This phase is not part of the v1 gate.** It begins only after Phase 5 archives and v1 is landed (Roadmap Sprint 11) **and after its v1.x prerequisite — universal decoration coverage (above) — lands**, which is what lets the POC's comment-less files (`package.json`, configs) be decoration-covered and reach the 100%-coverage bar. It adds no rows to the coverage matrix below.
...
- **The thesis constraint: zero hand-written application code.** All application source — including every test body under the pyramid suffixes — is produced by the pipeline via the **CLI `dusk implement` path** (which wires the post-hoc `gateWorktreeEdits` gate; the MCP write surface is gated-by-contract only and has no live file-writing engine). Human inputs are whitelisted: dialog responses (`dusk_author_continue`), `dusk_implement` requests, `dusk_resolve_livelock` / recovery-ladder resolutions, and commit review/merge approval. The constraint is **mechanically audited**: a trailer-audit script walks the POC's git history and verifies every commit touching application source carries the full v9 trailer set (`Intent`/`Bead-id`/`Verdict-id`/`Trace-id`/… per App. A.7) or is a merge of such commits.
  - **Two orthogonal axes — lead with the 2×2, never conflate them** (the cohesion seam with the universal-decoration-coverage prerequisite). The grounding example: **`package.json` is *trailer-exempt* yet *coverage-required*** — it may be `dusk init` scaffold (so its commit need not carry trailers) *and* must still carry a `package.json.intent` sidecar (so it counts toward coverage). The two axes answer independent questions — "did Dusk *produce* this file?" (provenance) vs "is this file *linked to intent*?" (coverage) — and the design classifies each file on both:
    - **Coverage axis** (governed by `decoration.ignore`, per the prerequisite): EVERY non-ignored file is decoration-covered — comment-bearing inline, comment-less (`package.json`, configs) via their `<file>.intent` sidecar. The *only* coverage-exempt files are the `decoration.ignore` globs (deps / generated output / secrets). So configs/manifests/migrations are **NOT coverage-exempt** — they carry sidecars and count toward the POC's 100%-coverage bar.
    - **Provenance axis** (governed by the trailer auditor): which commits must carry the full v9 trailer set. *Required* (pipeline-produced): all runtime source and all test bodies under the pyramid suffixes. *Trailer-exempt scaffold* (may be hand-/`dusk init`-authored, enumerated in the design): the `dusk init` output, the stack-config files, generated migrations, and the Vitest **infrastructure provisioning** (`globalSetup` + the e2e app-boot helper). Kept minimal. (Sprint 11; RFC §6.7, App. A.7, App. D.28.)
...
- **The full pyramid on real infrastructure** — the pipeline produces Verifier-validated integration + e2e **test bodies**; the Test Runner (vitest-only — it invokes `pnpm vitest run <files>`, it does **not** provision infrastructure) executes them against a live database and the app's real HTTP surface. The live infra is provisioned by the **POC's own Vitest `globalSetup`** (Postgres via docker-compose/testcontainers; an in-process app-boot helper for e2e) — *trailer-exempt* scaffold per the thesis-constraint bullet (still coverage-required, like any config) — which the runner's `vitest run` invocation honors (the dogfood-grade implement already runs worktree Vitest with `node_modules` symlinked, so the worktree inherits the config). The two-stage satisfaction + livelock machinery is live across both layers. This is the first real-code, real-infra pyramid run. (Sprint 11; RFC §3.4, §6.6.)
...
- **Friction → v1.x backlog** — the dogfood mechanics reused: dated JSONL under the POC's `.ia/observability/`, a **`PocReport`** — a new sibling schema in `@dusk/core-schema` modeled on `DogfoodReport`'s gating/exploratory two-section pattern (NOT a literal reuse of `DogfoodReport`, whose gating section is `.strict()`-locked to the four dogfood thresholds; the POC's hard gates differ and are more — see Cohesive landing) — separating hard gates from exploratory data; friction-driven role/skill edits land in the dusk repo as ordinary reviewed commits. (Sprint 11.)

### Greenfield foundations + robustness findings (arch-board, resolved during the build)

The first build runs surfaced two design questions; the architecture board resolved both (RFC App. D.24–D.27). They are recorded here as the contract for how greenfield is built and what in-build support work is in-scope:

- **The greenfield foundation IS authored intents, not an orchestration phase (App. D.24/D.25).** Project/module + tech-stack setup, app bootstrap, and the persistence layer are the **first sequence of intents** the author writes through `dusk_author` — never a synthesized foundation bead, bootstrap spawn, or canonical foundation Block (all rejected as orchestration coupling / framework tar-pit). The build authors **foundation intents first**, in dependency order (project/stack → bootstrap → persistence → behavior), then behavior intents. Robustness lives in the **Author dialog**: Stage-2 tension detection gains a general fifth class, **`prerequisite`** (App. D.25; RFC Ch. 5) — the request depends on an intent absent from the tree. It catches, at authoring time, a behavior intent depending on an unmet dependency (the greenfield foundation being the canonical instance) — the proactive complement to D.10's reactive mid-`dusk_implement` pause. A `prerequisite` is an ordinary surfaced tension handled by the existing transition; **no bootstrap state enters the orchestration**. This is friction-feedback work (classification + role-prompt/skill edits), exactly the kind this phase's cohesive landing already calls for.
- **In-scope orchestration robustness (App. D.26/D.27/D.33/D.34 — D11-style support fixes, each with a regression test + spec delta).** The greenfield/external-repo load exercised paths v1's in-monorepo dogfood never did: (1) external-repo gate-hook path resolution (`computeHookCommand`), session-snapshot base-ref fallback (`origin/main → main → HEAD`), and worktree creation requiring the resolved merge-base SHA (fail-loud, no `origin/main` default); (2) "a wall-clock overrun is a continue, not a discard" — a wall-clock timeout salvages the partial worktree draft and re-enters the short cycle instead of being misclassified as a transport failure that cold-retries and dies (no per-spawn turn cap — it would only force cold re-derivation); (3) **App. D.33** generalizes (2)'s lesson from the Engineer path to the **model-call** path — a deterministic `error_max_turns` / content-limit exit is classified non-transport (not cold-retried to a fatal `TransportLegFailure`) and surfaced as a returned `verifier_model_call_failed` / `task_tool_call_failed` at the `spawnSubAgent` seam, never an uncaught crash; `--tools ""` removes the tool surface as a defense-in-depth rate-improver (no correctness weight); (4) **App. D.34** is the *extraction* of (2)/(3)'s lesson into a governing principle (RFC §1.2.1) on its 4th instance — the deterministic core acts only on signals it authored or that are universal, the agentic bridge interprets the tech surface, and every boundary resolves to exactly one of `content | no_verdict | transport`, never inferring a verdict from silence. The driver was an empty/degraded Verifier under load (30/34 short-cycle Verifier calls returned 0-token responses at 25–74 s on `claude-sonnet-4-6`), looping ~21× re-drafting correct code (not the originally-reported vitest worker-pool flake — that framing was wrong and is superseded; the test runner was never reached). The confirmed live trigger (preserved POC traces, bead `bd_…005`) was the degraded response **silently coerced into a definite, falsely-converging verdict** — the verifier procedure's `?? false` / `?? "vague"` completeness collapse — slipping through the short-cycle's `!verdict || isDuskError` guard as valid *content* and converging on an empty failing set; the twin `parseModelResponse` → null → `recoverable: true` *downgraded* to `recoverable: false` at the short-cycle/Stage-1/long-cycle boundaries is a real but latent defect of the same family. Both are instances of inferring a verdict from silence. The fix adds a mechanical floor that preserves both honesty duals without re-coupling to a toolchain: the project's test command emits Dusk's *own* result schema via a project-side adapter/reporter (mirroring how code is decorated to map to intent schema), the core reads only that schema, and schema-absent output is interpreted agentically and routes to a finite, resumable infrastructure-recovery axis (`paused_infrastructure`, precedence `livelock > no_verdict > budget`, excluded from livelock reject-observations). These reshape no capability; they harden existing seams under the native greenfield load.
...
- **P6-T2 · The intent tree is born entirely through dialogs.**
  ...
  - **HOW** — Provenance is asserted against **durable records, not the dialog directory** (`dusk_author_finalize` destroys `.ia/runtime/dialogs/<id>/` per the Phase-4 contract, so live transcripts do not survive authoring): for every intent under the POC's `.ia/intents/`, assert a correlating **author-role event in the observability trace stream** (`traces.jsonl`, `role: author`) plus the finalize `intents_created` record naming it. (If the design wants the negotiation text retained for the published reference, it copies each transcript to a durable POC artifact *before* finalize — a Phase-6 design choice, not a runtime change.) Assert the tree contains ≥1 `polarity: negative` triple, ≥1 `compose: implies` intent with a closed-vocabulary antecedent, and pyramid children at unit + integration + e2e layers.
...
- **P6-T3 · Integration tests execute against live Postgres through the pipeline.**
  ...
  - **HOW** — A `dusk_implement` run whose intent set includes an `…/integration-tests` child produces Verifier-validated test bodies; with the POC's `globalSetup` providing live Postgres, the Test Runner's `vitest run` executes them against it → assert the `TestVerdict` satisfies the layer's `covers-*` triples and the commit carries `Test-Intent` + `Test-Verdict-id` trailers.
...
- **P6-T4 · E2e tests execute against the app's real HTTP surface through the pipeline.**
  ...
  - **HOW** — Same shape as P6-T3 for an `…/e2e-tests` child: with the POC's `globalSetup`/app-boot helper bringing up the HTTP surface, the Test Runner's `vitest run` executes the pipeline-produced e2e suite against it → assert the `TestVerdict` and trailers.
...
Every v1 commitment from the roadmap's "Architectural commitments locked for v1" table and the plan brief's "Hard expectations" list, mapped to the phase that lands it and a representative behavioral test. If a row has no phase, v1 is not done. **(The post-v1 changes — universal decoration coverage and Phase 6 — add no rows to this ledger; v1 completeness is determined by Phases 1–5 alone.)**
