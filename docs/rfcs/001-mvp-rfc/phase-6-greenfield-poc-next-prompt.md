# Handoff Prompt — Phase 6: Greenfield POC (the first v1.x change)
...
### 6.1 First: confirm v1 is landed

Check `openspec/changes/archive/` for the archived Phase-5 change. If it is not archived (the window opened 2026-06-25), **stop** and surface that — Phase 6 is post-v1 by contract. Do not start the greenfield build against an un-landed v1.
...
- **The "application source" predicate** for the trailer auditor — it must crisply separate *required* code (pipeline-produced, trailer-required) from *exempt scaffold* (may be hand-authored). Rule it explicitly in the design, enumerated, not improvised in the script:
  - *Required:* all runtime application source **and all test bodies** under the configured pyramid suffixes (`unit-tests`/`integration-tests`/`e2e-tests`).
  - *Exempt:* the `dusk init` output; `package.json`/`tsconfig`/`vitest.config`/Drizzle config; generated migrations; and the **Vitest infrastructure provisioning** (the `globalSetup` that spins up Postgres + the e2e app-boot helper). Keep the exempt set minimal and decorated where it is genuinely code. This is the honest line: the *test bodies* prove P6-T3/T4 were Dusk-produced; the *infra harness* that lets them run is scaffold.
...
1. Confirm the Phase-5 change is archived under `openspec/changes/archive/` (v1 is landed). If not, stop and say so.
